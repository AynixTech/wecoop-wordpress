...
use CustomFacebookFeed\Builder\CFF_Post_Set;
...
class CFF_Graph_Data
{
	...
	public function check_needded_videos($graph_data)
	{
		$posts_json = $graph_data->data
			? $graph_data->data
			: [];

		if  (count($posts_json) > 0) {
			if (count($posts_json) < $this->post_limit
				&& isset($graph_data->paging->next)
			) {
				$this->graph_url = $graph_data->paging->next;
				$graph_data = $this->get_remote_data($graph_data);
			}
		}
	...
	public function merge_responses($prev_response, $response)
	{
		$new_data = $response;
		array_push($prev_response->data, ...$new_data->data);
		$response->data = array_unique($prev_response->data, SORT_REGULAR);
		return $response;
	}

	public function filter_videos($posts_json, $prev_response = null)
	{
		$data = json_decode($posts_json);
		$videos = $data->data;
		$filtered_videos = CFF_Post_Set::check_published_videos($videos, $this->feed_settings);
		$data->data = $filtered_videos;
		if (!empty($prev_response)) {
			$data = $this->merge_responses($prev_response, $data);
		}
		$data = $this->check_needded_videos($data);

		return is_object($data) ? wp_json_encode($data) : $data;
	}
	...
	public function get_cache_seconds()
	{
		...
		$cache_time = (intval($this->feed_settings['cachetime']) < 1) ? 1 : intval($this->feed_settings['cachetime']);
	 ...
	 * @since 5.0
	 */
	public function get_remote_data($prev_response = null)
	{
		...
		if ($this->feed_settings['feedtype'] === 'videos') {
			$response = $this->filter_videos($response, $prev_response);
		}
