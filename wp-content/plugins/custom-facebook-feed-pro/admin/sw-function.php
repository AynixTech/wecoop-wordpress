...
function cff_get_processed_options($feed_options)
{
	if (isset($feed_options['feed'])) {
		...
		if (isset($feed_options['include'])) {
			$feed_options['include'] = is_array($feed_options['include']) ? implode(',', $feed_options['include']) : $feed_options['include'];
		}
		if (isset($feed_options['type'])) {
	...
	if (WPW_SL_ITEM_NAME == 'Custom Facebook Feed WordPress Plugin Smash') {
		...
		if (isset($cff_ext_options[ 'cff_extensions_carousel_active' ])) {
			$cff_ext_carousel_active_check = $cff_ext_options[ 'cff_extensions_carousel_active' ];
		}
		if (isset($cff_ext_options[ 'cff_extensions_reviews_active' ])) {
			$cff_extensions_reviews_active = $cff_ext_options[ 'cff_extensions_reviews_active' ];
		}
	}
	...
	if (! empty($options[ 'cff_show_links_type' ])) {
		...
		if (!isset($options[ 'cff_show_albums_type' ])) {
