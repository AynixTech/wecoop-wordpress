...
class CFF_Shortcode extends CFF_Shortcode_Display
{
	...
	public function cff_get_prev_url_parts($json_data_arr)
	{
		...
		if (substr($prev_urls_arr_safe, -3) == ', }') {
			$prev_urls_arr_safe = str_replace(", }", "}", $prev_urls_arr_safe);
		}
		return [
	...
	public function using_legacy_feed($feed_options)
	{
		...
		if (
			isset($cff_statuses['support_legacy_shortcode'])
	...
	public function get_settings_for_feed($feed_options)
	{
		...
		if (is_string($cff_includes)) {
			if (CFF_Utils::stripos($cff_includes, 'likebox') !== false) {
		...
		if (is_string($cff_excludes)) {
			if (CFF_Utils::stripos($cff_excludes, 'likebox') !== false) {
				$cff_show_like_box = false;
			}
		...
		if (
			! empty($settings['lightboxbgcolor'])
		...
		if (
			! empty($settings['beforedateenabled'])
		...
		if (
			! empty($settings['afterdateenabled'])
		...
		if (!empty($feed_options['disable-image-loading'])) {
			$style_options['enable_js_image_loading'] = false;
		}
		...
		$settings['cacheunit'] = !empty($feed_options['cacheunit']) ? $feed_options['cacheunit'] : 'minutes';
	...
	public function cff_get_processed_options($feed_options)
	{
		...
		// This used for the Divi and ELementor Builders
		$disable_js_loading		= isset($feed_options['disable-js-loading']) ? $feed_options['disable-js-loading'] : null;
		...
		if (empty($feed_options)) {
			...
			if ($disable_js_loading !== null) {
				$feed_options['disable-js-loading'] = true;
			}
			return $feed_options;
		}
		...
		if ($feed_options[ 'reviewshidenotext' ] == 'false') {
			$cff_reviews_no_text = false;
		}

		if ($feed_options['reviewsmethod'] == 'auto' && ( $cff_reviews_no_text || substr_count($feed_options[ 'reviewsrated' ], ',') < 4 )) {
			$feed_options['reviewsmethod'] = 'all';
		}
	...
	public static function get_single_event_data($eventID, $access_token)
	{
		...
		$event_json_url = 'https://graph.facebook.com/v23.0/' . $eventID . '?fields=cover,place,name,owner,start_time,end_time,timezone,id,comments.summary(true){message,created_time},description,event_times&access_token=' . $access_token . $cff_ssl;
		...
		if (isset($event_object->name)) {
			$description_text .= $event_object->name . ' ';
		}
		if (isset($event_object->place->location->city)) {
			$description_text .= $event_object->place->location->city . ' ';
		}
		if (isset($event_object->place->location->country)) {
			$description_text .= $event_object->place->location->country . ' ';
		}
		if (isset($event_object->place->location->street)) {
			$description_text .= $event_object->place->location->street . ' ';
		}
		if (isset($event_object->place->name)) {
			$description_text .= $event_object->place->name . ' ';
		}
		if (isset($event_object->description)) {
			$description_text .= $event_object->description;
		}
	...
	public static function cff_get_next_url_parts($json_data_arr)
	{
		...
		$next_urls_arr_safe .= '}';
		// If the array ends in a comma then remove the comma
		if (substr($next_urls_arr_safe, -3) == ', }') {
			$next_urls_arr_safe = str_replace(", }", "}", $next_urls_arr_safe);
		}
	...
	public function cff_get_post_set_html($feed_options, $json_data_arr, $original_atts = array())
	{
		...
		if (!empty($feed_options['disable-js-loading'])) {
			$style_options['enable_js_image_loading'] = false;
		}
		// Active extensions
		...
		if (($cff_show_albums_type && $cff_albums_source == 'photospage') && !$cff_show_links_type && !$cff_show_video_type && !$cff_show_photos_type && !$cff_show_status_type && !$cff_show_event_type) {
		...
		if (!empty($cff_link_title_size) && $cff_link_title_size != 'inherit') {
			$cff_link_title_styles =  'style="font-size:' . esc_attr($cff_link_title_size) . 'px;"';
		}
		...
		if ($cff_post_style == 'boxed' || $cff_post_bg_color_check) {
			...
			if ($cff_post_bg_color_check) {
				$cff_item_styles .= 'background-color: #' . esc_attr($cff_post_bg_color) . '; ';
			}
			if (isset($cff_post_rounded) && $cff_post_rounded !== '0' && !empty($cff_post_rounded)) {
				$cff_item_styles .= 'border-radius: ' . esc_attr($cff_post_rounded) . 'px; ';
			}
			$cff_item_styles .= '"';
		}
		...
		// If the Page ID contains a query string at the end then remove it
		if (CFF_Utils::stripos($page_id, '?') !== false) {
			$page_id = substr($page_id, 0, strrpos($page_id, '?'));
		}
		...
		foreach ($page_ids as $page_id) {
				...
				if (empty($json_data->data)) {
					...
					foreach ($json_data->data as $event) {
						...
						if ($cff_filter_string != '') {
							...
							$string_in_desc = true;
							if (CFF_Utils::cff_stripos_arr_filter($cff_event_address_string, $cff_filter_strings_array) === false) {
								$string_in_address = false;
							}
							if (CFF_Utils::cff_stripos_arr_filter($description, $cff_filter_strings_array) === false) {
						...
						if ($cff_exclude_string != '') {
							...
							if (CFF_Utils::cff_stripos_arr_filter($cff_event_address_string, $cff_exclude_strings_array) !== false) {
								$string_in_address = true;
							}
							if (CFF_Utils::cff_stripos_arr_filter($description, $cff_exclude_strings_array) !== false) {
								$string_in_desc = true;
							}
						...
						if ($cff_past_events) {
							$cur_time = strtotime($event_time) - 1; // If past events then can't use current time, so use the first event start date and subtract 1 so is before.
						}
						...
						if (!empty($event_time)) {
							...
							if ($cff_multiple_date_count > 0) {
								...
								foreach ($event_time_arr as $event_time_unix => $event_time_item) {
									...
									if (strlen($event_time_unix) == 24) {
										$event_time_unix = substr($event_time_unix, 0, -5);
									}
									...
									if (isset($event_time_item['end_time'])) {
										...
										// If timezone migration is enabled then remove last 5 characters from end time
										if (strlen($cff_event_end_time) == 24) {
											$cff_event_end_time = substr($cff_event_end_time, 0, -5);
										}
										if ($ev_index === 0) {
							...
							if (isset($event->end_time)) {
								$cff_event_date .= '<span class="cff-end-date" ' . wp_kses($cff_event_date_styles, ['"']) . '> - ' . CFF_Utils::cff_eventdate($e_date, $cff_event_date_formatting, $cff_event_date_custom, $cff_event_timezone_offset, $cff_timezone) . '</span>';
							}
							if ($cff_multiple_date_count > 0) {
								$cff_event_date .= '<a href="javascript:void(0);" class="cff-more-dates">+' . $cff_multiple_date_count . '</a>';
							}
						...
						if ($cff_post_bg_color_check || $cff_post_style == "boxed") {
							$cff_post_item .= ' cff-box';
						}
						$cff_post_item .= ' cff-new';
						if ($event_time < $cur_time) {
							$cff_post_item .= ' cff-past';
						}
						if ($cff_box_shadow) {
							$cff_post_item .= ' cff-shadow';
						}
						$cff_post_item .= '"';
						$cff_post_item .= ' data-cff-timestamp="';
						if (isset($event->start_time)) {
							$cff_post_item .= strtotime($event->start_time);
						}
						$cff_post_item .= '"';
						...
						if ($cff_show_media) {
							...
							$lightbox_tag = !empty($feed_options['lightbox']) && $feed_options['lightbox'] === 'on'
								? 'span' : 'a';
							$cff_post_item .= '<div class="cff-media-wrap">';
							$cff_post_item .= '<' . $lightbox_tag .' class="cff-photo nofancybox';
							if ($crop_event_pic) {
								$cff_post_item .= ' cff-crop';
							}
							$cff_post_item .= '" href="' . esc_url($event_link) . '" ' . wp_kses($target, ['"']) .  wp_kses($cff_nofollow, ['"']) . wp_kses($media_src_set_att, ['"']) . '><img src="' . CFF_Display_Elements_Pro::get_media_placeholder($pic_big, $style_options) . '" class="cff-feed-image" data-orig-source="' . esc_url($pic_big) . '" alt="' . htmlspecialchars($cff_alt_text) . '" data-querystring="' . esc_attr($cff_picture_querystring) . '" ';
							if ($cff_no_event_img) {
								$cff_post_item .= 'data-cff-no-event-img-large="' . esc_attr($pic_big_lightbox) . '"';
							}
							$cff_post_item .= ' /></' . $lightbox_tag .'></div>';
						}
						...
						if ($cff_show_event_details) {
							if (!empty($location)) {
								$cff_post_item .= '<p class="cff-location" ' . wp_kses($cff_event_details_styles, ['"']) . '>';
							}

							$cff_event_link_color_html = '';
							if (isset($cff_event_link_color) && !empty($cff_event_link_color) && $cff_event_link_color != '#') {
								$cff_event_link_color_html = 'style="color: #' . esc_attr($cff_event_link_color) . '"';
							}

							if (!empty($venue_id)) {
								$cff_post_item .= '<a href="' . esc_url($venue_link) . '" ' . wp_kses($target, ['"']) .  wp_kses($cff_nofollow, ['"']) . ' ' . $cff_event_link_color_html . '>';
							}
							if (!empty($location)) {
								$cff_post_item .= '<b class="cff-event-place">' . $location . '</b>';
							}
							if (!empty($venue_id)) {
								$cff_post_item .= '</a>';
							}
							if (!empty($venue_street)) {
								$cff_post_item .= '<span class="cff-event-street">' . $venue_street . '</span>';
							}
							if (!empty($venue_city)) {
								$cff_post_item .= '<span class="cff-event-city">' . $venue_city . ',</span>';
							}
							if (!empty($venue_state)) {
								$cff_post_item .= '<span class="cff-event-state"> ' . $venue_state . '</span>';
							}
							if (!empty($venue_zip)) {
								$cff_post_item .= '<span class="cff-event-zip">' . $venue_zip . '</span>';
							}
							...
							} elseif (1 === preg_match('~[0-9]~', $location) && strlen($location) > 10) {
							...
							if (!empty($map_url)) {
								$cff_post_item .= ' <a href="' . esc_url($map_url) . '" ' . wp_kses($target, ['"']) .  wp_kses($cff_nofollow, ['"']) . ' ' . $cff_event_link_color_html . ' class="cff-event-map-link">' . stripslashes(__($cff_map_text, 'custom-facebook-feed')) . '</a>';
							}
						...
						if (!empty($description) && CFF_Utils::stripos($cff_includes, 'text')) {
							...
							if (!empty($title_limit)) {
								if (strlen($description) > $title_limit) {
									$cff_post_item .= 'data-char="' . esc_attr($title_limit) . '" ';
								}
							}
							...
							$cff_linebreak_el = '<br />';
							if ($cff_format_issue) {
								$cff_linebreak_el = '<div alt="cff-linebreak-placeholder" aria-hidden="true" class="cff-linebreak" ></div>';
							}
							...
							if (!empty($title_limit)) {
								if (strlen($description) > $title_limit) {
									$cff_post_item .= '<span class="cff-expand">... <a href="#" ' . $cff_posttext_link_color_html . '><span class="cff-more">' . stripslashes(__($cff_see_more_text, 'custom-facebook-feed')) . '</span><span class="cff-less">' . stripslashes(__($cff_see_less_text, 'custom-facebook-feed')) . '</span></a></span>';
								}
							}
						...
						if (empty($cff_interested_text) || $cff_interested_text == '') {
							$cff_interested_text = 'interested';
						}
						if (empty($cff_going_text) || $cff_going_text == '') {
						...
						if ($interested_count_num > 0 || $attending_count_num > 0) {
							...
							if ($interested_count_num > 0) {
								$cff_post_item .= $interested_count . ' ' . stripslashes(wp_kses_post($cff_interested_text));
							}
							if ($interested_count_num > 0 && $attending_count_num > 0) {
								$cff_post_item .= ' &nbsp;&middot;&nbsp; ';
							}
							if ($attending_count_num > 0) {
								$cff_post_item .= $attending_count . ' ' . stripslashes(wp_kses_post($cff_going_text));
							}
							$cff_post_item .= '</div>';
						}
						...
						if ($cff_show_link) {
							...
							$cff_share_email = 'mailto:?subject=Facebook&amp;body=' . urlencode($event_link) . '%20-%20' . rawurlencode(strip_tags($cff_event_title) . ' - ' . strip_tags($cff_event_date));
								...
								// Buy tickets link
							if ($ticket_uri !== '' && $cff_show_link) {
								$cff_post_item .= '<a href="' . esc_url($ticket_uri) . '" target="_blank" ' . $cff_nofollow . ' ' . wp_kses_post($cff_link_styles) . '>' . stripslashes(__($cff_buy_tickets_text, 'custom-facebook-feed')) . '</a><span class="cff-dot" ' . $cff_link_styles . '>&middot;</span>';
							}

								$shortened_description = ! empty($description) ? CFF_Utils::cff_maybe_shorten_text($description) : CFF_Utils::cff_maybe_shorten_text($cff_event_title);
								// View on Facebook link
							if ($cff_show_facebook_link) {
								$cff_post_item .= '<a class="cff-viewpost" href="' . esc_url($event_link) . '" ' . $target . $cff_nofollow . ' ' .  wp_kses($cff_link_styles, ['"']) . '>' . $cff_facebook_link_text .  '</a>';
							}
							...
							if ($cff_show_facebook_share) {
								...
								if ($cff_show_facebook_link) {
									$cff_post_item .= '<span class="cff-dot" ' .  wp_kses($cff_link_styles, ['"']) . '>&middot;</span>';
								}
						...
						if ($cff_show_post !== false) {
							$cff_posts_array = CFF_Utils::cff_array_push_assoc($cff_posts_array, $event_time, $cff_post_item);
						}
					} // End the loop
				} // End empty() check
			...
			if (!$cff_events_only || ($cff_events_only && $cff_events_source == 'timeline')) {
				...
				if (isset($FBdata->data)) {
					if (
						! \cff_main_pro()->cff_error_reporter->are_critical_errors(false)
				...
				if ($fbdata_string) {
					foreach ($fbdata_string as $news) {
						...
						if (isset($news->attachments->data[0]->media_type)) {
							$cff_post_type = $news->attachments->data[0]->media_type;
						}
								...
								if (isset($news->attachments->data[0]->subattachments)) {
									...
									if (strpos($link, 'photos/a.') !== false || $news->type == 'album') {
										...
										if (isset($albumLinkArr1[1])) {
											...
											if (strpos($albumLinkArr1[1], '/') !== false) {
												...
												if (count($albumLinkArr2) > 1) {
													if (is_numeric(intval($albumLinkArr2[1]))) {
														array_shift($albumLinkArr2);
													}
												}
											}
										}
										...
										if (isset($albumLinkArr1[1]) || $news->type == 'album') {
											...
											if (CFF_Utils::stripos($album_link, $album_link_check) !== true) {
												$album_link = $link;
											}
										}
									}
						...
						if ($cff_hide_supporter_posts && isset($news->privacy->description)) {
							if ($news->privacy->description == 'Supporters only') {
								$cff_show_post = false;
							}
						}
						...
						if ($cff_filter_string != '') {
							...
							if (CFF_Utils::cff_stripos_arr_filter($cff_text_to_be_filtered, $cff_filter_strings_array) === false) {
								$string_in_post_text = false;
							}
							if (CFF_Utils::cff_stripos_arr_filter($description_text, $cff_filter_strings_array) === false) {
								$string_in_desc = false;
							}
						...
						if ($cff_exclude_string != '') {
							...
							if (CFF_Utils::cff_stripos_arr_filter($cff_text_to_be_filtered, $cff_exclude_strings_array) !== false) {
								$string_in_post_text = true;
							}
							if (CFF_Utils::cff_stripos_arr_filter($description_text, $cff_exclude_strings_array) !== false) {
								$string_in_desc = true;
							}
						...
						if (($prev_post_message == $pm) && ($prev_post_link == $pl) && ($prev_post_description == $pd) && !isset($news->call_to_action->type)) {
							$cff_show_post = false;
						}
							...
							if ($cff_offset_show_post) {
								if (!$cff_ext_multifeed_active && !$show_all_reviews) {
									...
									if (intval($this->feed_options['offset']) > 0) {
										...
										if ($i == (intval($show_posts) + intval($this->feed_options['offset']))) {
											break;
										}
									} else {
								...
								if (isset($news->backdated_time)) {
									$post_time = $news->backdated_time; // If the post is backdated then use that as the date instead
								}
								...
								if (!$cff_photos_only && !$cff_videos_only && !($cff_albums_only && $cff_albums_source == 'photospage') && empty($cff_album_id) && !$cff_reviews) {
									...
									if ($cff_post_type == 'video' || $cff_post_type == 'music' || $cff_post_type == 'link') {
										...
										if ($cff_post_type == 'video' && isset($news->name) && !$cff_is_video_embed) {
											if (!$cff_title_link) {
												$cff_video_name .= '<a href="' . esc_url($link) . '" ' .  wp_kses($target, ['"']) .  wp_kses($cff_nofollow, ['"']) . ' ' . wp_kses($cff_posttext_link_color_html, ['"']) . '>';
											}
									...
									if (!empty($news->description) || !empty($news->caption)) {
										...
										if ($cff_post_type == 'event' || $cff_is_video_embed || $cff_soundcloud || $cff_spotify) {
											$cff_description = '';
										}
									}
									...
									if ($news->type == 'video') {
										if (!empty($cff_description) && $cff_description != '') {
											if ((!empty($post_text) && $post_text != '') && !empty($cff_video_name)) {
												$post_text .= '<br /><br />';
											}
											$post_text .= $cff_video_name;
										}
									...
									if ($cff_show_event_title || $cff_show_event_details) {
										...
										if ($cff_post_type == 'event') {
											...
											if ($cff_show_media && $cff_timeline_event_image) {
												...
												if ($cff_event_has_cover_photo) {
													$cff_timeline_event_photo .= ' cff-has-cover';
												}
												$cff_alt_text = apply_filters('cff_img_alt', $cff_alt_text);
											...
											$cff_event_date = '';
											if (!empty($event_time)) {
												$cff_event_date = '<span class="cff-date" ' . wp_kses($cff_event_date_styles, ['"'])  . '><span class="cff-start-date">' . CFF_Utils::cff_eventdate(strtotime($event_time), $cff_event_date_formatting, $cff_event_date_custom, $cff_event_timezone_offset, $cff_timezone) . '</span>' . $event_end_time . '</span>';
											}
											...
											if ($cff_show_event_title && !empty($event_object->name)) {
												...
												if ($cff_event_title_link) {
													$cff_event .= '<a href="' . esc_url($link) . '" ' . wp_kses($target, ['"']) . wp_kses($cff_nofollow, ['"']) . '>';
												}
											...
											if ($cff_show_event_details) {
												// Location
												if (!empty($event_object->place->name)) {
													$cff_event .= '<span class="cff-where" ' . wp_kses($cff_event_details_styles, ['"']) . '>' . $event_object->place->name . '</span>';
												}
												// Description
												if (!empty($news->message) || isset($event_object->description)) {
													...
													if (isset($event_object->description)) {
														$description_safe = htmlentities($event_object->description, ENT_QUOTES, 'UTF-8');
													}
													...
													if ($post_text == $cff_description_tagged) {
														$post_text = '';
													}
											...
											if (!empty($post_text) && $post_text != '') {
												$post_text .= $cff_linebreak_el . $cff_linebreak_el;
											}
											$post_text .= $cff_event;
										}
									}
									...
									if ($cff_post_type == 'note') {
										...
										if ($cff_note_media_src && $cff_show_media) {
											...
											if ($cff_note_media_src) {
												$cff_note_picture_html .= ' cff-has-cover';
											}
											$cff_alt_text = apply_filters('cff_img_alt', $cff_alt_text);
									...
									if (isset($news->call_to_action->value->link) && !empty($news->call_to_action->value->link)) {
										...
										if ($cff_cta_link[0] == '/') {
											...
											if (strpos($cff_cta_link, 'http') === false && strpos($cff_cta_link, 'tel:') === false) {
												$cff_cta_link = 'http://' . $cff_cta_link;
											}
										}
										...
										switch ($cff_button_type) {
											case 'GET_DIRECTIONS':
												...
												if (!isset($cff_translate_get_directions_text) || empty($cff_translate_get_directions_text)) {
											...
											case 'SHOP_NOW':
												...
												if (!isset($cff_translate_shop_now_text) || empty($cff_translate_shop_now_text)) {
											...
											case 'MESSAGE_PAGE':
												...
												if (!isset($cff_translate_message_page_text) || empty($cff_translate_message_page_text)) {
											...
											case 'LEARN_MORE':
												...
												if (!isset($cff_translate_learn_more_text) || empty($cff_translate_learn_more_text)) {
										...
										if ($cff_button_type == 'MESSAGE_PAGE') {
											$cff_cta_link = 'https://m.me/' . $cff_from_id;
										}

										// Add the button to the post if the text isn't "NO_BUTTON"
										if ($cff_button_type != 'NO_BUTTON') {
											$cff_post_text .= '<p class="cff-cta-link" ' . wp_kses($cff_title_styles, ['"']) . '><a href="' . esc_url($cff_cta_link) . '" ' . $cff_nofollow . ' target="_blank" data-app-link="' . esc_url($cff_app_link) . '" ' . wp_kses($cff_posttext_link_color_html, ['"']) . '>' . $cff_cta_button_text . '</a></p>';
										}
									}
										...
										if ($cff_soundcloud) {
											...
											if (isset($news->picture) || isset($news->attachments->data[0]->media->image)) {
												if (!empty($news->picture)) {
													...
													$cff_link_image = true;
													$cff_one_x_one = '1x1.';
													if (CFF_Utils::stripos($news->picture, $cff_one_x_one) == true || empty($news->picture)) {
														$cff_link_image = false;
													}
												}
													...
													if ($cff_full_link_images && isset($news->attachments->data[0]->subattachments->data)) {
														...
														foreach ($news->attachments->data[0]->subattachments->data as $attachment_item) {
															if (isset($attachment_item->media->image->src)) {
																...
																$cff_shared_link .= '<img src="' . CFF_Display_Elements_Pro::get_media_placeholder($attachment_item->media->image->src, $style_options) . '" class="cff-feed-image" alt="Link image" data-orig-source="' . esc_attr($attachment_item->media->image->src) . '" />';
																$cff_shared_link .= '</div>';
															}
														}
														// Put default image at end
														$cff_shared_link .= '<div class="cff-link-slider-item cff-final-item">';
														$cff_shared_link .= '<img src="' . CFF_Display_Elements_Pro::get_media_placeholder($picture, $style_options) . '" class="cff-feed-image" data-orig-source="' . $full_picture . '" alt="Link image" />';
														$cff_shared_link .= '</div>';
														...
														$media_src_set_att = ' data-img-src-set="' . esc_attr(CFF_Utils::cff_json_encode(CFF_Parse_Pro::get_media_src_set($news))) . '"';
														$cff_shared_link .= '<div class="cff-link" data-link="' . esc_url($link) . '"' . wp_kses($media_src_set_att, ['"']) . ' data-full="' . esc_url($full_picture) . '">';
														$cff_shared_link .= '<img  src="' . CFF_Display_Elements_Pro::get_media_placeholder($picture, $style_options) . '" class="cff-feed-image" data-orig-source="' . $picture . '" alt="Link thumbnail" />';
														$cff_shared_link .= '</div>';
													}
												}
											}
											...
											if (isset($cff_link_title_color) && !empty($cff_link_title_color) && $cff_link_title_color != '#') {
												$cff_link_title_color_html = 'style="color: #' . esc_attr($cff_link_title_color) . '"';
											}

											if (isset($news->name)) {
												$cff_shared_link .= '"><' . CFF_Utils::check_custom_htmltag($cff_link_title_format) . ' class="cff-link-title" ' . wp_kses($cff_link_title_styles, ['"']) . '>
													<a href="' . esc_url($link) . '" ' . wp_kses($target, ['"']) . wp_kses($cff_nofollow_referrer, ['"']) . ' ' . wp_kses($cff_link_title_color_html, ['"']) . '>' . htmlentities($news->name, ENT_QUOTES, 'UTF-8') . '</a>
											...
											if (isset($cff_link_url_color) && !empty($cff_link_url_color) && $cff_link_url_color != '#') {
												$cff_link_url_color_html = 'color: #' . str_replace('#', '', esc_attr($cff_link_url_color)) . ';';
											}
											if ($cff_link_url_size != 'inherit' && !empty($cff_link_url_size)) {
												$cff_link_url_size_html = 'font-size:' . esc_attr($cff_link_url_size) . 'px;';
											}

											$cff_link_styles_html = '';
											if (strlen($cff_link_url_color_html) > 1 || strlen($cff_link_url_size_html) > 1) {
												$cff_link_styles_html = 'style="';
											}
											if (strlen($cff_link_url_color_html) > 1) {
												$cff_link_styles_html .= $cff_link_url_color_html;
											}
											if (strlen($cff_link_url_size_html) > 1) {
												$cff_link_styles_html .= $cff_link_url_size_html;
											}
											if (strlen($cff_link_url_color_html) > 1 || strlen($cff_link_url_size_html) > 1) {
												$cff_link_styles_html .= '"';
											}
											...
											if (!empty($cff_link_caption)) {
												$cff_shared_link .= '<p class="cff-link-caption" ' . wp_kses($cff_link_styles_html, ['"']) . '>' . $cff_link_caption . '</p>';
											}
											if ($cff_show_desc) {
												// Truncate desc
												if (!empty($body_limit)) {
												...
												if (isset($cff_link_desc_color) && !empty($cff_link_desc_color) && $cff_link_desc_color != '#') {
													$cff_link_desc_color_html = 'color: #' . str_replace('#', '', esc_attr($cff_link_desc_color)) . ';';
												}
												if ($cff_link_desc_size != 'inherit' && !empty($cff_link_desc_size)) {
													$cff_link_desc_size_html = 'font-size:' . esc_attr($cff_link_desc_size) . 'px;';
												}

												$cff_link_desc_styles_html = '';
												if (strlen($cff_link_desc_color_html) > 1 || strlen($cff_link_desc_size_html) > 1) {
													$cff_link_desc_styles_html = 'style="';
												}
												if (strlen($cff_link_desc_color_html) > 1) {
													$cff_link_desc_styles_html .= $cff_link_desc_color_html;
												}
												if (strlen($cff_link_desc_size_html) > 1) {
													$cff_link_desc_styles_html .= $cff_link_desc_size_html;
												}
												if (strlen($cff_link_desc_color_html) > 1 || strlen($cff_link_desc_size_html) > 1) {
													$cff_link_desc_styles_html .= '"';
												}
												...
												if ($description_text != $cff_link_caption) {
													$cff_shared_link .= $cff_link_description;
												}
											}
									...
									if ($news->type == 'photo' || $news->type == 'album' || $news->type == 'offer' || ( $cff_featured_post_active && !empty($this->feed_options['featuredpost']) && isset($news->images) )) {
										...
										if ($cff_image_size == 'small' && isset($news->picture)) {
											$picture = $news->picture;
										}
										...
										if ($cff_multi_img_layout) {
											if ($cff_album && isset($news->attachments->data[0]->subattachments)) {
												...
												if ($cff_attachment_total >= 12) {
													$cff_attachment_total_display = ''; // If there's more than 12 then we can't display the number as we can only count up to 12 subattachments in the posts API.
												}
												...
												foreach ($news->attachments->data[0]->subattachments->data as $attachment_item) {
													...
													if (!$is_product_attachment) {
														...
														if ($a == 0) {
															...
															if ($cff_main_img_height > $cff_main_img_width) {
																$cff_portrait = true;
															}
														}
														...
														if ($a > 0 && $a < 4) {
															...
															if ($cff_img_count == 3) {
																$cff_img_attachments .= '<span class="cff-more-attachments"><span>+' . $cff_attachment_total_display . '</span></span>';
															}

															$cff_img_attachments .= '<img src="' . CFF_Display_Elements_Pro::get_media_placeholder($attachment_src, $style_options) . '" data-orig-source="' . $attachment_src . '" alt="Image attachment" class="cff-multi-image cff-feed-image" /></span>';
															$cff_img_count++;
														}
														$a++;
													}
												...
												if ($cff_img_count > 2) {
													$cff_img_attachments_html = '<span class="cff-img-attachments">';
												}
												$cff_img_attachments_html .= $cff_img_attachments;
												if ($cff_img_count > 2) {
													$cff_img_attachments_html .= '</span>';
												}
											}
										}
										...
										if ($cff_img_count > 1) {
											$cff_media .= ' cff-multiple cff-img-layout-' . $cff_img_count;
										}
										if ($cff_portrait) {
											$cff_media .= ' cff-portrait';
										}
										$cff_media .= ' nofancybox" ';
										if ($cff_img_count > 1) {
											$cff_media .= 'style="max-width: ' . esc_attr($cff_main_img_width) . 'px;" ';
										}
										...
										if ($cff_album) {
											$cff_media .= '<span class="cff-album-icon"></span>';
										}
										...
										if ($cff_img_count > 1) {
											$cff_media .= '<span class="cff-img-wrap cff-main-image cff-crop">';
										}
										$cff_alt_text = apply_filters('cff_img_alt', $cff_alt_text);

										$cff_media .= '<img src="' . CFF_Display_Elements_Pro::get_media_placeholder($picture, $style_options) . '" data-orig-source="' . $picture . '" alt="' . htmlspecialchars(substr($cff_alt_text, 0 , 20)) . '" data-querystring="' . $cff_picture_querystring . '" data-ratio="' . round($cff_main_img_width / $cff_main_img_height, 3) . '" class="cff-multi-image cff-feed-image" />';
										if ($cff_img_count > 1) {
											$cff_media .= '</span>';
										}
									...
									if ($news->type == 'swf') {
										...
										$cff_media .= '" ' . $target . $cff_nofollow . ' ' . wp_kses($cff_lightbox_sidebar_atts, ['"']) . ' ' . wp_kses($media_src_set_att, ['"']) .  '><img src="' . CFF_Display_Elements_Pro::get_media_placeholder($picture, $style_options) . '" class="cff-feed-image" /></a>';
									}
									...
									if ($cff_post_type == 'video' && !$cff_soundcloud && !$cff_spotify) {
										...
										if ($youtube || $youtu || $youtubeembed) {
											...
											if (!empty($cff_video_height)) {
												$cff_media_video .= 'style="height: ' . esc_attr($cff_video_height) . '"';
											}
											$cff_media_video .= '>';
											...
											if (isset($PostID[1])) {
												$cff_post_id = $PostID[1];
											}

											if ($cff_video_action == 'facebook') {
												$cff_media_video .= '<a href="https://facebook.com/' . $cff_post_id . '" target="_blank" ' . $cff_nofollow . ' class="cff-media-overlay"></a>';
											}
											// Add image as it's needed for lightbox ordering for embedded iframe videos
											if (! CFF_GDPR_Integrations::doing_gdpr($this->feed_options)) {
												$cff_media_video .= '<img src="' . $picture . '" alt="Iframe Video Thmbnail" class="cff-iframe-img cff-feed-image" />';
											}

											$cff_media_video .= $code . '</div>';

										// Check whether it's a vimeo
										} elseif (CFF_Utils::stripos($url, $vimeo) !== false) {
											...
											if (!empty($news->source)) {
												...
												$cff_media_video .= '<img src="' . CFF_Display_Elements_Pro::get_media_placeholder($picture, $style_options) . '" alt="Iframe Video Thmbnail" class="cff-iframe-img cff-feed-image" />';
										...
										// Else use a video player
											...
											if (isset($news->attachments->data[0]->media->image->src)) {
												$poster = $news->attachments->data[0]->media->image->src;
											}
														...
														if (!$is_product_attachment) {
															...
															if ($a > 0 && $a < 4) {
																...
																if ($cff_disable_lightbox) {
																	$cff_img_attachments .= "<a href='https://www.facebook.com/" . $page_id . "/posts/" . $PostID[1] . "' " . $target . $cff_nofollow . " style='width:100%; float:left; clear: both; z-index: 1; position: relative;'>";
																}
																...
																if ($cff_disable_lightbox) {
																	$cff_img_attachments .= '</a>';
																}

																$cff_img_attachments .= '<img src="' . CFF_Display_Elements_Pro::get_media_placeholder($attachment_src, $style_options) . '" class="cff-feed-image" alt="Image attachment" /></span>';
																$cff_img_count++;
															}
															$a++;
														}
											...
											if ($cff_img_count > 1) {
												$cff_media_video .= ' cff-multiple cff-img-layout-' . $cff_img_count;
											}
											if ($cff_portrait) {
												$cff_media_video .= ' cff-portrait';
											}
											if ($swf_file) {
												$cff_media_video .= ' cff-swf';
											}
											$cff_media_video .= '"' . $cff_lightbox_sidebar_atts . ' data-cff-video-link="' . esc_url($link) . '" data-cff-video-player="' . esc_attr($cff_video_player) . '" data-cff-live="' . esc_url($cff_live_video) . '">';
											...
											if (!$cff_disable_lightbox) {
												...
												$cff_media_video .= '<img class="cff-poster cff-feed-image" src="' . CFF_Display_Elements_Pro::get_media_placeholder($poster_img, $style_options) . '" data-orig-source="' . esc_url($poster_img) . '" data-cff-full-img="' . esc_url($poster) . '" alt="' . htmlspecialchars($vid_title) . '" data-querystring="' . $cff_picture_querystring . '" data-cff-video="' . esc_url($vid_link) . '" style="float: left;" data-ratio="' . round($cff_main_img_width / $cff_main_img_height, 3) . '"' . wp_kses($media_src_set_att, ['"']) . ' />';
												...
												if ($cff_video_action == 'facebook') {
													...
													if (isset($news->source) && $cff_mp4_check && $cff_video_player == 'standard') {
														...
														if ($this->feed_options['loadmore']) {
															$cff_media_video .= '<img class="cff-poster cff-feed-image" src="' . esc_url($poster_img) . '" alt="' . htmlspecialchars($vid_title) . '" data-querystring="' . $cff_picture_querystring . '" style="position: absolute; top: 0; left: 0; z-index: 7;" data-ratio="' . round($cff_main_img_width / $cff_main_img_height, 3) . '" />';
														}
														...
														$cff_media_video .= '<img class="cff-poster cff-feed-image" src="' . CFF_Display_Elements_Pro::get_media_placeholder($poster_img, $style_options) . '" alt="' . htmlspecialchars($vid_title) . '"' . $media_src_set_att . ' />';
														$cff_media_video .= '</div>';
													}
												}
											}
											...
											if ($cff_img_count > 1) {
												$cff_media_video .= '</span>';
											}
											if ($cff_disable_lightbox && $cff_img_count < 2) {
												$cff_media_video .= "<a href='https://www.facebook.com/" . $page_id . "/posts/" . $PostID[1] . "' " . $target . $cff_nofollow . " style='width:100%; float:left; clear: both; z-index: 1; position: relative;'>";
											}
									...
									if ($cff_post_type == 'event' && isset($event_object)) {
										$news = $event_object;
									}
									...
									if ($feed_options[ 'lightboxcomments' ] === 'false' || $feed_options['lightboxcomments'] == false) {
									...
									$cff_show_meta = false;
									if (CFF_Utils::stripos($cff_includes, 'social') !== false) {
										$cff_show_meta = true;
									}
									// Exclude string for meta
									$cff_excludes = $feed_options[ 'exclude' ];
									if (CFF_Utils::stripos($cff_excludes, 'social') !== false) {
										$cff_show_meta = false;
									}
									...
									if ($cff_post_type == 'link') {
										$cff_post_type_class = 'cff-link-item';
									}
									if ($cff_post_type == 'event') {
										$cff_post_type_class = 'cff-timeline-event';
									}
									if ($cff_post_type == 'photo' || $cff_post_type == 'album') {
										$cff_post_type_class = 'cff-photo-post';
									}
									if ($cff_post_type == 'video' && !$cff_soundcloud) {
										$cff_post_item .= 'cff-video-post';
									}
									if ($cff_soundcloud || $cff_spotify || $cff_post_type == 'music') {
									...
									if ($cff_post_type == 'swf') {
										$cff_post_type_class = 'cff-swf-post';
									}
									if ($cff_post_type == 'offer') {
										$cff_post_type_class = 'cff-offer-post';
									}
									...
									if (isset($news->from->name)) {
										$cff_post_item .= CFF_Utils::cff_to_slug($news->from->name);
									}
									...
									if (isset($news->created_time)) {
										$cff_post_item .= strtotime($news->created_time);
									}
									$cff_post_item .= '"';
									if (isset($news->backdated_time)) {
										$cff_post_item .= ' data-cff-backdated="' . strtotime($news->backdated_time) . '"';
									}
									$cff_post_item .= ' data-object-id="' . $object_id . '"';
									if (isset($news->from->id)) {
										$cff_post_item .= ' data-cff-from="' . $news->from->id . '"';
									}
									if ($this->feed_options['loadcommentsjs'] == 'true') {
										$cff_post_item .= 'data-comments-js="true"';
									}
									...
									if ($cff_show_media && $cff_media_position == 'above') {
										if ($cff_post_type == 'event') {
											$cff_media = $cff_timeline_event_photo;
										}
										if ($news->type == 'note') {
											$cff_media = $cff_note_picture_html;
										}
									...
									if ($cff_show_date && $cff_date_position == 'above' && !$cff_is_video_embed) {
										...
										// POST TEXT
									if ($cff_show_text === true  && !$cff_is_video_embed) {
										$cff_post_item .= $cff_post_text;
									}
									...
									// DATE BELOW
									if ($cff_show_date && $cff_date_position == 'below' && ( (!$cff_thumb_layout && !$cff_half_layout) || $cff_is_video_embed)) {
										$cff_post_item .= $cff_date;
									}
									...
									if ($cff_show_post) {
										...
										if ($cff_is_group) { // Groups need to use token in the request:
											...
											if (isset($news->cover_photo)) {
												...
												} elseif (isset($news->cover_photo->id)) {
										...
										$cff_post_item .= ' cff-new" ';
										$cff_post_item .= ' data-page-id="' . $page_id . '"';
										$cff_post_item .= ' data-post-id="cff_' . $page_id . '_' .$news->id . '"';
										$cff_post_item .= ' data-cff-timestamp="';
										if (isset($news->created_time)) {
											$cff_post_item .= strtotime($news->created_time);
										}
								...
								if ($cff_album_active && !empty($cff_album_id)) {
									...
									if ($cff_filter_string != '') {
										...
										$string_in_post_text = true;
										$string_in_desc = true;
										if (CFF_Utils::cff_stripos_arr_filter($cff_album_desc, $cff_filter_strings_array) === false) {
											$cff_show_post = false;
										}
									}
									...
									if ($cff_exclude_string != '') {
										...
										$string_in_post_text = false;
										$string_in_desc = false;
										if (CFF_Utils::cff_stripos_arr_filter($cff_album_desc, $cff_exclude_strings_array) !== false) {
											$cff_show_post = false;
										}
									}
								...
								if ($cff_videos_only && empty($cff_album_id)) {
									...
									*/
									...
									if (isset($news->published) && $news->published === false) {
										$cff_show_post = false;
									}
									if ($cff_show_post) {
										...
										if (isset($news->source) && $cff_video_player === 'standard' ) {
										...
										if ($cff_video_action == 'post' && $cff_disable_lightbox) {
											...
											if (! CFF_GDPR_Integrations::doing_gdpr($this->feed_options)) {
												...
												$cff_post_item .= '<img class="cff-poster cff-feed-image"
													src="' . (!empty($poster_img) ? CFF_Display_Elements_Pro::get_media_placeholder($poster_img, $style_options) : '' ). '"
													alt="' . (!empty($vid_title) ? htmlspecialchars($vid_title) : '' ). '"' . $media_src_set_att . '
												/>';
												$cff_post_item .= '</div>';
										} else {
											$cff_post_item .= '<a href="https://facebook.com/' . $news->id . '" class="cff-album-cover cff-video" ' . wp_kses($target, ['"']) . wp_kses($cff_nofollow, ['"']) . $media_src_set_att . ' id="' . $news->id . '" data-source="' . $cff_vid_source . '">' . CFF_Display_Elements_Pro::get_icon('play', '', 'cff-playbtn') . '<img src="' . CFF_Display_Elements_Pro::get_media_placeholder($poster, $style_options) . '" class="cff-feed-image" data-orig-source="' . $poster . '" alt="' . htmlspecialchars($poster_alt) . '" data-querystring="' . $cff_picture_querystring . '" /></a>';
										}
										...
										if ($cff_show_video_name && !empty($video_name)) {
											$cff_post_item .= '<h4><a href="https://facebook.com/' . $news->id . '" ' . wp_kses($target, ['"']) . wp_kses($cff_nofollow, ['"']) . '>' . $video_name . '</a></h4>';
										}
								...
								if ($cff_photos_only && empty($cff_album_id)) {
									...
									if ($cff_filter_string != '') {
										...
										$string_in_post_text = true;
										$string_in_desc = true;
										if (CFF_Utils::cff_stripos_arr_filter($cff_caption, $cff_filter_strings_array) === false) {
											$cff_show_post = false;
										}
									}
									...
									if ($cff_exclude_string != '') {
										...
										$string_in_post_text = false;
										$string_in_desc = false;
										if (CFF_Utils::cff_stripos_arr_filter($cff_caption, $cff_exclude_strings_array) !== false) {
											$cff_show_post = false;
										}
									}
									...
									if ($cff_show_post) {
										// Get full size image
										if (!empty($picture)) {
											$picture = 'https://graph.facebook.com/' . $id . '/picture?type=normal&width=9999&height=9999';
										}
										if (!empty($picture) && isset($news->images)) {
											$full_picture = $news->images[0]->source;
										}
				...
				if ($cff_photos_only && empty($cff_album_id)) {
					...
					if ($cff_ext_multifeed_active && count($page_ids) > 1) {
						shuffle($cff_posts_array);
					}
				} elseif ($cff_album_active && !empty($cff_album_id) || $cff_videos_only || $cff_albums_only) {
		...
		if ($cff_events_only && $cff_events_source == 'eventspage') {
			...
			if (!empty($this->feed_options['offset'])) {
				$cff_posts_array = array_slice($cff_posts_array, intval($this->feed_options['offset']));
			}
			// If no events then add notice
			if (empty($cff_posts_array)) {
				$cff_posts_array = CFF_Utils::cff_array_push_assoc($cff_posts_array, 1, '<p class="cff-no-events">' . stripslashes(__($cff_no_events_text, 'custom-facebook-feed')) . '</p>');
			}
		}
		...
		// If it's an events feed and pagination is enabled then render all the events to the page so pagination can be done in JS
		} elseif ($cff_load_more && ( ($cff_events_only && $cff_events_source == 'eventspage') || ( $cff_reviews && $show_all_reviews ) )) {
			foreach ($cff_posts_array as $post) {
				$cff_content .= $post;
			}
		} else {
			...
			foreach ($cff_posts_array as $post) {
				...
				if (strpos($post, 'cff-empty-album') !== false) {
					$p--;
				}
	...
	public function cff_process_submitted_resize_ids()
	{
		...
		if (!isset($atts['feed']) || !wp_verify_nonce($nonce, 'cff-locator-nonce-' . $post_id . '-' . $atts['feed'])) {
		...
		// If an access token is set in the shortcode then set "use own access token" to be enabled
		if (isset($atts['accesstoken'])) {
			$atts['ownaccesstoken'] = 'on';
		}
	...
	public function cff_get_new_posts()
	{
		...
		$nonce = isset($_POST['locator_nonce']) ? sanitize_text_field(wp_unslash($_POST['locator_nonce'])) : '';
		if (!wp_verify_nonce($nonce, 'cff-locator-nonce-' . $post_id . '-' . $shortcode_data['feed'])) {
