...
class CFF_Utils
{
			...
			if (!empty($FBdata)) {
				...
				if (isset($FBdata->error)) {
					...
					if (false !== $feed_cache->get($cache_type . '_backup')) {
						...
						$prefix = '{';
						if ($posts_json !== null && substr($posts_json, 0, strlen($prefix)) == $prefix) {
							$posts_json = substr($posts_json, strlen($prefix));
						}
	...
	public static function fetch_header_data($page_id, $cff_is_group, $access_token, $cff_cache_time, $cff_multifeed_active, $data_att_html)
	{
		...
		if ($cff_multifeed_active && strpos($page_id, ',') !== false) {
			...
			if (is_array($header_page_id_arr)) {
				$header_page_id = $header_page_id_arr[0];
			}
		}
		...
		$header_details_json_url = 'https://graph.facebook.com/v23.0/' . $header_page_id . '?fields=id,picture.height(150).width(150),cover,name,link' . $page_only_fields . '&access_token=' . $header_access_token;
		// Get the data
