...
class CFF_Post_Set
{
	...
	public function fetch()
	{
		...
		$facebook_feed = new \CustomFacebookFeed\CFF_Feed_Pro($transient, true);
	...
	public static function check_published_videos($data_array, $settings)
	{
		$result_posts = [];
		$i = 0;
		$videos_to_show = $settings['videotypetoshow'];

		foreach ($data_array as $post) {
			if (isset($post->status->publishing_phase->publish_time)
				&& $i < $settings['num']
			) {
				$video_type = self::video_type($post);
				if ( $videos_to_show === 'both'
					|| $video_type === $videos_to_show
				) {
		...
		$data_array = $result_posts;
		return $data_array;
	}


	public static function video_type($video)
	{
		$video = is_array($video) ? $video : (array) $video;
		if (!isset($video['embed_html'])) {
			return false;
		}

		$embedHtml = $video['embed_html'];
		if (strpos($embedHtml, '/reel/') !== false
			|| strpos($embedHtml, '%2Freel%2F') !== false
		) {
			return "reels";
		} elseif (strpos($embedHtml, '/videos/') !== false
			|| strpos($embedHtml, '%2Fvideos%2F') !== false
		) {
			return "videos";
		}
		return false;
	...
	public static function builder_to_general_settings_convert($builder_settings)
	{
		...
		if (isset($builder_settings['sources']) && is_array($builder_settings['sources'])) {
			...
			if (! empty($sources_setting) && sizeof($sources_setting) > 0) {
		...
		if (
			empty($builder_settings['pagetoken'])
	...
	public static function legacy_to_builder_convert($atts = array())
	{
		...
		foreach ($settings_with_multiples as $multiple_key) {
			if (
				isset($processed_settings[ $multiple_key ])
