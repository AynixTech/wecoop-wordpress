...
class CFF_Source
{
	...
	public static function builder_update()
	{
		if (check_ajax_referer('cff-admin', 'nonce', false)) {
			...
			$source_data = array(
				...
				'privilege' => isset($_POST['privilege']) ? sanitize_text_field(wp_unslash($_POST['privilege'])) : ''
			);

			if (!empty($_POST['name'])) {
				$source_data['name'] = sanitize_text_field(wp_unslash($_POST['name']));
			}

			$return = CFF_Source::process_connecting_source_data($source_data);
			$response = json_decode($return, true);
			if (isset($_POST['iCalUrl'], $_POST['privilege']) && $_POST['privilege'] === 'events' && !empty($_POST['iCalUrl'])) {
				if (!isset($response['success']) || $response['success'] !== false) {
					$ical_url = sanitize_text_field(wp_unslash($_POST['iCalUrl']));
					$ical_url_valid = CFF_Events_Parser::check_ical_url($ical_url);
					if (!$ical_url_valid) {
						 $return_html = '<div class="cff-groups-connect-actions sb-alerts-wrap"><div class="sb-alert">
                            ...
                            <span><strong>' . __('There was a problem connecting getting iCal URL. Please make sure your iCal URL is correct.') . '</strong></span>
                        </div></div>';
					...
					CFF_Events_Parser::update_ical_url($source_data['id'], $ical_url);
				}
			}
		...
		if (check_ajax_referer('cff-admin', 'nonce', false)) {
			...
			if (isset($_POST['sourcesList']) && !empty($_POST['sourcesList']) && is_array($_POST['sourcesList'])) {
				foreach ($_POST['sourcesList'] as $single_source) :
					$source_data = array(
						'access_token' => sanitize_text_field(wp_unslash($single_source['access_token'])),
						'id' => sanitize_text_field(wp_unslash($single_source['account_id'])),
						// 'name' => isset($single_source['name']) ? sanitize_text_field(wp_unslash($single_source['name'])) : '',
						'name'		   => isset($single_source['username']) ? sanitize_text_field(wp_unslash($single_source['username'])) : '',
						'type' => sanitize_text_field(wp_unslash($_POST['type'])),
						'privilege' => isset($single_source['privilege']) ? sanitize_text_field(wp_unslash($single_source['privilege'])) : '',
					);
					if (isset($single_source['location'])) {
						$source_data['location'] = sanitize_text_field(wp_unslash($single_source['location']));
					}
	...
	public static function get_page()
	{
		...
		$args = array('page' => sanitize_text_field(wp_unslash($_POST['page'])));
	...
	public static function get_featured_post_preview()
	{
		...
		$query_args = array(
			'id' => sanitize_text_field(wp_unslash($_POST['source_id']))
		);
		...
		if ($encryption->decrypt($access_token)) {
			$access_token = $encryption->decrypt($access_token);
		}

		$url_or_post_id = sanitize_text_field(wp_unslash($_POST['url_or_id']));
		$id = CFF_Source::extract_id($url_or_post_id, 'album');

		if (isset($id)) {
			$feed_type = isset($_POST['feed_type']) ? sanitize_text_field(wp_unslash($_POST['feed_type'])) : null;
			$data = CFF_Source::fetch_featured($id, sanitize_text_field(wp_unslash($_POST['source_id'])), $access_token, $feed_type);
			echo $data;
		} else {
			echo '{"error":{"message":"No valid ID found"}}';
		}
	 ...
	 * @since 4.0
	 */
	public static function fetch_featured($post_id, $source_id, $access_token, $feed_type = null)
	{
		...
		$featured_album_url = 'https://graph.facebook.com/v23.0/' . $post_id . '?fields=id,created_time,name,count,cover_photo{source,id},link&access_token=' . $access_token . '&limit=1';

		if ($feed_type === 'album') {
			$response =  wp_safe_remote_get($featured_album_url);
			$return = '{}';
	...
	public static function fetch_playlist($playlist_id, $access_token)
	{
		...
		$url = 'https://graph.facebook.com/v23.0/' . $playlist_id . '/videos/?access_token=' . $access_token . '&fields=published,source,updated_time,created_time,title,description,embed_html,format{picture}&locale=en_US&limit=5';
	...
	public static function get_connection_urls($is_settings = false)
	{
		...
		$admin_url_state 	= $is_settings ?
								admin_url('admin.php?page=cff-settings') :
	...
	public static function maybe_source_connection_data()
	{
		...
		if (
			...
			$reporter = \CustomFacebookFeed\CFF_Utils::cff_is_pro_version()
						? cff_main_pro()->cff_error_reporter
						: cff_main()->cff_error_reporter;
			...
			if ($return) {
				...
				return array(
					'error' => __('Unable to connect to Facebook to retrieve account information.', 'custom-facebook-feed')
				);
			}
		}
		return false;
	...
	public static function retrieve_available_pages($access_token)
	{
		...
		if (isset($pages_data_arr['data'])) {
			$data = $pages_data_arr['data'];

			$data = self::merge_locations($data, $user_id_data_arr, $access_token);
	...
	public static function merge_locations($data)
	{
		$return = array();

		foreach ($data as $single_account) {
			$return[] = $single_account;
			$locations_data = self::get_page_locations($single_account['id'], $single_account['access_token']);
			if (! empty($locations_data)) {
				foreach ($locations_data as $location) {
					$url = 'https://graph.facebook.com/v15.0/' . $location['id'] . '/?fields=name,location&access_token=' . $single_account['access_token'];
					$single_location_data     = \CustomFacebookFeed\CFF_Utils::cff_fetchUrl($url);
					$decoded_single = json_decode($single_location_data, true);
					if (! empty($decoded_single['location'])) {
						$new_connected_account = $single_account;

						$details_array = array();
						if (! empty($decoded_single['location']['city'])) {
							$details_array[] = $decoded_single['location']['city'];
						}
						if (! empty($decoded_single['location']['state'])) {
							$details_array[] = $decoded_single['location']['state'];
						}
						if (! empty($decoded_single['location']['country'])) {
							$details_array[] = $decoded_single['location']['country'];
						}
						$new_connected_account['name'] = $single_account['name'];
						$new_connected_account['id']   = $location['id'];
						if (sizeof($details_array) > 0) {
							$new_connected_account['location'] = implode(', ', $details_array);
						}
	 ...
	 * @return array|mixed
	 */
	public static function get_page_locations($page_id, $access_token)
	{
		$url = 'https://graph.facebook.com/v15.0/' . $page_id . '/locations?fields=location_page_id&limit=50&access_token=' . $access_token;
		$locations_data     = \CustomFacebookFeed\CFF_Utils::cff_fetchUrl($url);
		$return = array();

		if (! empty($locations_data)) {
			$decoded = json_decode($locations_data, true);

			if (! empty($decoded['data'][0])) {
				$return = array_merge($return, $decoded['data']) ;
			}
			if (! empty($decoded['paging']['cursors']['after'])) {
				$paging = '&after=' . $decoded['paging']['cursors']['after'];
				$url = $url . $paging;
				$locations_second_page_data     = \CustomFacebookFeed\CFF_Utils::cff_fetchUrl($url);
				if (! empty($locations_second_page_data)) {
					$decoded = json_decode($locations_second_page_data, true);
					if (! empty($decoded['data'][0])) {
						$return = array_merge($return, $decoded['data']) ;
					}
				}
	...
	public static function update_or_insert($source_data)
	{
		...
		if (isset($source_data['info'])) {
			...
			if (is_object($source_data['info']) || is_array($source_data['info'])) {
				if (isset($source_data['location'])) {
					$source_data['info']->location = $source_data['location'];
				}
	...
	public static function extract_id($url_or_post_id, $type)
	{
		...
		if ($type === 'album') {
			if (strpos($url_or_post_id, '/') === false) {
				...
				$id_part = $things[1];
				$url_end_parts = explode('&', $id_part);
				$id = $url_end_parts[0];
			} elseif (strpos($url_or_post_id, 'set=oa.') !== false) {
				$things = explode('set=oa.', $url_or_post_id);
			...
			if (strpos($url_or_post_id, '/') === false) {
				$id = sanitize_text_field(wp_unslash($url_or_post_id));
	 ...
	 * @since 4.2.0
	 */
	public static function get_source_info($source_id)
	{
		$query_args = array(
