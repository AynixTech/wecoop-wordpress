...
class CFF_Feed_Saver
{
	...
	/**
	 * Quick DB Setting Update
	 *
	 * @return false|int
	 *
	 * @since 4.0
	 */
	public function quick_setting_update()
	{
		$this->sanitize_and_sort_data();
		return $this->update();
	}
		...
		if (! empty($source_query)) {
			foreach ($source_query as $source) {
				...
				$return['sources'][] = array(
					...
					'access_token' => $is_export === true ? stripslashes($encryption->encrypt($source['access_token'])) : stripslashes($encryption->decrypt($source['access_token'])),
	...
	{
		{
			...
			$defaults = array(
				...
				'eventstimezoneoffsetchanged' => false,
	...
	/**
	 * Retrieves and get the Feed Settings & Options As it is
	 *
	 * @return array|bool
	 *
	 * @since 4.0
	 */
	public function get_feed_options()
	{
		return $this->feed_options;
	}
}
